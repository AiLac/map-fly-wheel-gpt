...
主 Skill 管理数据根；下表各子目录均相对 `docs/super-business-flow/`。工具文件拆分不改变 Skill 名称，也不增加仅负责目录的 Skill。

| Skill | 主管目录 | 完成物 |
| --- | --- | --- |
| `super-business-flow` | `docs/super-business-flow/` | 输入归一、阶段调度、检查点、汇总 |
...
目录拆分依据用户最新明确要求，替代早期所有非 `.cac` 文件统一放在 `docs/business-flow/` 的约定。业务仓库路径和 `.temp/run/super-business-flow/<run-id>/` 不变。迁移必须保留人工修订及历史证据；不通过全局字符串替换改写不可变版本，不承诺跨版本自动恢复未完成 run。[迁移说明](layout-migration.md)
