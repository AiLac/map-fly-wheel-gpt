...
public final class RunService {
    private static final String BASE = Data.DATA_ROOT + "/";
    private static final String RUNS = Data.RUN_ROOT + "/";
    ...
    private static JsonNode repos(Path project) throws IOException {
        ...
        Data.require(Files.isRegularFile(path), "Configure docs/super-business-flow/project/repos.yaml first; the tool will not scan itself");
    ...
    private static void collect(Path root, boolean repository, FileConsumer visitor) throws IOException {
        ...
        Files.walkFileTree(root, new SimpleFileVisitor<>() {
            @Override public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs) {
                ...
                if (name.equals(".git") || (repository && (Set.of(".temp", "target", "build", "node_modules", ".idea", ".vscode").contains(name) || Set.of(Data.DATA_ROOT, Data.TOOLKIT_ROOT).contains(relative)))) return FileVisitResult.SKIP_SUBTREE;
    ...
    private static JsonNode help() {
        return Data.object().put("schema_version", 1).put("usage", "run (--class FQCN [--url URL ...] | --url URL ... | --all | --resume RUN_ID) [--max-parallel-tasks N] [--phase-limit PHASE=N] [--max-retries N] [--task-timeout-seconds N]")
            ...
            .put("guide", "docs/super-business-flow-tookit/design/run-contract.md");
    }
}
