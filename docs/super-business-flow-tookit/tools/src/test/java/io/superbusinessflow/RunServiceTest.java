...
class RunServiceTest {
    ...
    private void configure(int modules) throws Exception {
        ...
        Data.write(project.resolve("docs/super-business-flow/project/repos.yaml"),cfg);
    }
    ...
    @Test void relativeMultilineClasspathChangesAreObserved() throws Exception {
        ...
        Path cfg=project.resolve("docs/super-business-flow/project/repos.yaml");JsonNode repos=Data.read(cfg);((ObjectNode)repos.path("repos").get(0).path("modules").get(0)).put("classpath_file","build/deps.cp");Data.write(cfg,repos);
    ...
    @Test void memoryPinsImmutableVersionAndReadTimestampsDoNotInvalidateRun() throws Exception {
        configure(1);Path evidence=project.resolve("docs/super-business-flow/evidence/source.txt");Data.writeText(evidence,"Known scoped fact.\n");
        ObjectNode entry=Data.object().put("schema_version",1).put("id","navigation").put("type","navigation").put("statement","Known scoped fact").put("state","verified");entry.set("scope",Data.array().add("project:*"));entry.set("source_refs",Data.array().add(Data.object().put("path","docs/super-business-flow/evidence/source.txt").put("sha256",Data.fingerprint(evidence))));
        ...
        String id=command("--all").path("run_id").asText();Path current=project.resolve("docs/super-business-flow/memory/entries/navigation.md"),immutable=project.resolve("docs/super-business-flow/memory/versions/navigation/r000001.md");
    ...
    @Test void workspaceConfigDiscoverySeparatesToolkitAndPublishedOutputsFromInputs() throws Exception {
        configure(1);
        Path config = project.resolve("docs/super-business-flow/project/repos.yaml");
        ObjectNode repos = (ObjectNode) Data.read(config);
        ObjectNode repo = (ObjectNode) repos.path("repos").get(0);
        repo.put("path", ".");
        ((ObjectNode) repo.path("modules").get(0)).set("source_roots", Data.array().add("repo/module0/src"));
        Data.write(config, repos);
        String id = command("--all").path("run_id").asText();
        Data.writeText(project.resolve("docs/super-business-flow-tookit/examples/application.yaml"), "example: toolkit\n");
        Data.writeText(project.resolve("docs/super-business-flow/inventory/application.yaml"), "example: generated\n");
        assertFalse(command("--resume", id).path("invalidated").asBoolean());
        assertTrue(Files.exists(project.resolve(".temp/run/super-business-flow/" + id + "/state.json")));
        Data.writeText(project.resolve("docs/super-business-flow/frameworks/demo/rules.yaml"), "schema_version: 1\nstatus: candidate\n");
        assertTrue(command("--resume", id).path("invalidated").asBoolean());
    }
    ...
    @Test void stageAdoptionRequiresExactChangesAndInvalidatesSuccessors() throws Exception {
        ...
        Path rules=project.resolve("docs/super-business-flow/frameworks/demo/rules.yaml");Data.writeText(rules,"schema_version: 1\nstatus: candidate\n");
