...
| 目录 | 归属 | 更新方式 |
| --- | --- | --- |
| `docs/super-business-flow-tookit/` | 工具发行内容：`tools/`、`schemas/`、`templates/`、`design/`、`examples/` 及使用说明 | 随工具版本更新 |
| `docs/super-business-flow/` | 工程配置、RPC 规则、端点映射、扫描事实、业务文档、记忆和证据 | 由项目持续积累；工具升级保留原内容 |
| `.cac/` 中的 `super-business-flow*` 文件 | 本工具的 Skill、command 和 agent 集成 | 与工具包配套更新，保留其他宿主配置 |
| `.temp/run/super-business-flow/<run-id>/` | 临时任务状态和恢复检查点 | 由一次运行管理 |

工具根使用约定的 `super-business-flow-tookit` 名称；主 Skill 及其工作目录仍为 `super-business-flow`，各子 Skill 后缀对应数据根内的职责目录，无需新增 Skill。分析产物写入数据根或临时运行目录；Maven 构建自身的 `tools/target/` 仍在工具目录内。

升级只更新工具根与本工具的 `.cac` 集成，不重新复制初始项目数据。来自旧版 `docs/business-flow/` 的安装先按 [目录迁移说明](design/layout-migration.md) 分类迁移；不能对已发布证据和历史版本做全局路径替换。[项目数据入口](../super-business-flow/README.md)
