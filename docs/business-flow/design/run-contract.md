...
以下 `flow` 表示 `java -jar docs/business-flow/tools/target/business-flow-tools.jar`；可使用 `tools/bf.ps1` 或 `tools/bf.sh` 缩短命令。项目根由当前目录或 `--project ROOT` 指定。
  ...
  - id: caller
    ...
    modules:
      - id: application
        ...
        classpath_file: ../map-fly-wheel-gpt/docs/business-flow/project/local/caller-application.cp
```

上例假设工具工作区目录名是 map-fly-wheel-gpt；实际按你的目录填写。建议把 classpath 文件统一生成到工具项目的 `docs/business-flow/project/local/`，并将其真实位置登记到 classpath_file；配置也接受绝对路径。PowerShell 示例：

```powershell
New-Item -ItemType Directory -Force docs/business-flow/project/local
$flowClasspath = Join-Path (Resolve-Path docs/business-flow/project/local).Path 'caller-application.cp'
...
`.temp/run/super-business-flow/<run-id>/state.json` 是原子更新的权威状态。snapshot.json、tasks.json、frontier.json、questions.yaml、decisions.yaml、resume.md 是可重建投影；history 保留旧快照/失效任务，tasks/<id>/ 是分配给某个子任务的输出目录。运行目录自动创建忽略规则；无需把大体量临时状态提交 Git。过程设计、被采纳的发现和证据必须发布到 docs/business-flow。
...
flow task adopt --run-id ID --file docs/business-flow/project/local/adopt.json
```
...
先调用 memory retrieve/verify；只在派发前或无活动任务的阶段屏障执行 `task use-memory --run-id ID --file <JSON>`。文件为 `{schema_version:1,entries:[{id,revision,path,sha256}]}`，path 必须是 `docs/business-flow/memory/versions/<id>/rNNNNNN.md` 的不可变版本。工具重新核验当前记忆状态与源证据，拒绝 stale、pending、unavailable 或不匹配版本。读取导致的 last_used_at 变化不会改写已固定的版本。新任务只读本次实际采用的记忆；单条固定后要改用其他版本则新建运行。索引不是证据。
