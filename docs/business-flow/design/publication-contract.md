...
```powershell
java -jar docs/business-flow/tools/target/business-flow-tools.jar evidence --source ../caller/src/main/java/example/TsInnerClient.java --symbol 'example.TsInnerClient#searchByText(FusionRequestDTO)' --line-start 12 --line-end 20
```

返回 `{schema_version:1,id,path,sha256,source:{path,sha256,line_start,line_end,symbol}}`。外层 path 指向项目内 `docs/business-flow/evidence/<hash>/source.txt`，外层 SHA 是摘录内容的指纹；source 中保留原文件全量指纹和位置。同目录 `record.json` 是证据元数据。先持久化相关摘录，再将返回对象放入图的 evidence 数组。不要复制无关配置或整个仓库。持久化哈希证明摘录完整性，不能证明业务解释正确。

## 图与发布

```powershell
java -jar docs/business-flow/tools/target/business-flow-tools.jar validate --file docs/business-flow/scenarios/text-search/graph.json --schema docs/business-flow/schemas/graph.schema.json
java -jar docs/business-flow/tools/target/business-flow-tools.jar publish --graph docs/business-flow/scenarios/text-search/graph.json --scenario-id text-search
# 更新已有发布必须带 manifest 中当前 revision
java -jar docs/business-flow/tools/target/business-flow-tools.jar publish --graph docs/business-flow/scenarios/text-search/graph.json --scenario-id text-search --expected-revision 1
```
