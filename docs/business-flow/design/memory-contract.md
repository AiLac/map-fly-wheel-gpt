...
| 路径 | 职责 |
| --- | --- |
| `docs/business-flow/memory/entries/<id>.md` | 当前条目，YAML frontmatter 与 Markdown 正文 |
| `docs/business-flow/memory/versions/<id>/r000001.md` | 不可变内容版本，含删除墓碑 |
| `docs/business-flow/memory/index.json` | 可重建的元数据索引，不包含长正文 |
| `docs/business-flow/evidence/` | 跨运行持久化的原始证据；发布记忆前保存 |
| `docs/business-flow/.project.lock` | 与发布器共用的短时 OS 文件锁 |
...
`memory put --file` 接受 YAML、JSON，或 Markdown frontmatter 加正文。输入文件保存在 `docs/business-flow/` 的相应候选目录；运行内未发布候选可位于约定的 `.temp/run/super-business-flow/<run-id>/`。只有原始证据引用禁止指向 `.temp`。
...
state: pending
source_refs:
  - path: docs/business-flow/evidence/ts-client/Source.java
    sha256: "<真实文件字节的 64 位 SHA-256；示例占位符不可直接执行>"
canonical_refs:
  - docs/business-flow/frameworks/example-rpc/protocol.md
supersedes: []
basis:
...
证据路径可以使用 `/`，工具也接受 Windows `\` 分隔符；不接受绝对路径。兄弟 Git 仓库可用项目相对 `../repo-a/...`，但必须匹配 `project/repos.yaml` 中显式登记的仓库。程序检查真实路径与符号链接的归属，允许 `evidence/` 或登记仓库内的原始文件；拒绝 `.temp`、`.git`、记忆自身以及 `docs/business-flow/` 内非 evidence 的生成知识作为原始证据。依赖源码或未提交的源文件应先保存到持久 evidence，并记录原始出处；仅有当前工作树路径的记忆，在切换版本或仓库缺失时可能暂时不可用。
...
以下 `flow` 代表 `java -jar docs/business-flow/tools/target/business-flow-tools.jar`；Windows PowerShell 传入空格路径时使用双引号。

```text
flow memory put --file docs/business-flow/memory-candidate.yaml
flow memory put --file docs/business-flow/memory-candidate.yaml --expected-revision 1
