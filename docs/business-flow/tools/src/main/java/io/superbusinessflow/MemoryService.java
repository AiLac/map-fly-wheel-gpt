...
public final class MemoryService {
    private static final String ROOT = "docs/business-flow/memory";
    ...
    public static JsonNode execute(Path project, List<String> args) throws Exception {
        ...
        try (AutoCloseable ignored = Data.lock(root.resolve("docs/business-flow/.project.lock"))) {
    ...
    private static Path sourcePath(Path project, String relative, List<Path> registered) throws IOException {
        ...
        Path governance = canonicalPath(project.resolve("docs/business-flow"));
        Path evidence = canonicalPath(project.resolve("docs/business-flow/evidence"));
    ...
    private static List<Path> registeredRoots(Path project) throws IOException {
        Path config = project.resolve("docs/business-flow/project/repos.yaml");
    ...
    private static int defaultLimit(Path project) throws IOException {
        Path settings = project.resolve("docs/business-flow/project/settings.yaml");
